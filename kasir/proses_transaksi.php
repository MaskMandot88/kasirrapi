...
if ($metode_bayar === 'Hutang' && !kasirrapi_feature_allowed($subscription, 'piutang')) {
    die('Transaksi Hutang/Piutang tersedia mulai paket Plus.');
}
...
try {
    ...
    $transactionLimit = (int)($subscription['max_transactions_per_month'] ?? 0);
    if ($transactionLimit > 0) {
        ...
        if (kasirrapi_limit_reached($transactionLimit, (int)$stmtLimit->fetchColumn())) {
            throw new Exception('Batas transaksi bulanan paket Anda sudah tercapai. Upgrade ke Basic untuk transaksi unlimited.');
        }
    }
    ...
    $discountResult = discounts_calculate($pdo, $tenant_id, $detail_items, $metode_bayar);
    $diskon = min($subtotal_transaksi, (float)$discountResult['diskon']);
    $total = max(0, $subtotal_transaksi - $diskon);
