...
$piutang_allowed = kasirrapi_feature_allowed($subscription, 'piutang');
    ...
    <div class="kasir-sticky-total app-card overflow-hidden">
        ...
        <div class="p-4 md:p-5">
            <form id="form_transaksi"
                ...
                <div id="panel_ringkasan_diskon"
                     class="bg-slate-950 border border-slate-800 rounded-2xl p-3 text-sm space-y-2">
                    <div class="flex justify-between gap-3">
                        <span class="text-slate-400">Subtotal</span>
                        <strong id="teks_subtotal_ringkas">Rp 0</strong>
                    </div>
                    <div class="flex justify-between gap-3">
                        <span class="text-slate-400">Diskon Otomatis</span>
                        <strong id="teks_diskon_ringkas" class="text-emerald-300">Rp 0</strong>
                    </div>
                    <div id="daftar_diskon_ringkas" class="hidden pt-2 border-t border-slate-800 text-xs text-emerald-200 space-y-1"></div>
                </div>
...
let ringkasanDiskon = { subtotal: 0, diskon: 0, total: 0, items: [] };
let timeoutHitungDiskon = null;
...
function renderKeranjang() {
    ...
    ringkasanDiskon = { subtotal: total, diskon: 0, total: total, items: [] };
    updateTampilanDiskon();
    document.getElementById('data_keranjang_input').value = JSON.stringify(keranjang);

    jadwalkanHitungDiskon();
    ubahMetodeBayar();
}

function jadwalkanHitungDiskon() {
    clearTimeout(timeoutHitungDiskon);
    timeoutHitungDiskon = setTimeout(hitungDiskonOtomatis, 180);
}

function hitungDiskonOtomatis() {
    if (keranjang.length === 0) {
        ringkasanDiskon = { subtotal: 0, diskon: 0, total: 0, items: [] };
        updateTampilanDiskon();
        return;
    }

    fetch('api_diskon.php', {
        method: 'POST',
...
function updateTampilanDiskon() {
    const metode = document.getElementById('metode_bayar').value;
    const inputBayar = document.getElementById('input_bayar');

    document.getElementById('teks_subtotal_ringkas').innerText = 'Rp ' + formatRupiah(ringkasanDiskon.subtotal);
    document.getElementById('teks_diskon_ringkas').innerText = '- Rp ' + formatRupiah(ringkasanDiskon.diskon);
    document.getElementById('teks_total').innerText = 'Rp ' + formatRupiah(ringkasanDiskon.total);
    document.getElementById('total_tagihan_input').value = ringkasanDiskon.total;

    if (metode === 'QRIS' || metode === 'Transfer') {
        inputBayar.value = ringkasanDiskon.total;
    } else if (metode === 'Hutang' && (!inputBayar.value || Number(inputBayar.value) > ringkasanDiskon.total)) {
        inputBayar.value = 0;
    }

    const list = document.getElementById('daftar_diskon_ringkas');
    if (!ringkasanDiskon.items.length) {
        list.classList.add('hidden');
        list.innerHTML = '';
    } else {
        list.classList.remove('hidden');
        list.innerHTML = ringkasanDiskon.items.map(item => {
            return `<div class="flex justify-between gap-2"><span>${escapeHtml(item.nama || 'Diskon')}</span><strong>-Rp ${formatRupiah(item.nilai || 0)}</strong></div>`;
        }).join('');
    }
