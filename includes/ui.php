...
if (!function_exists('ui_icon')) {
    function ui_icon($name, $class = 'w-5 h-5') {
        $icons = [
            ...
            'diskon' => '<svg xmlns="http://www.w3.org/2000/svg" class="'.$class.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M20 12V7a2 2 0 0 0-2-2h-5L5 13l6 6 9-7Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M16 8h.01"/><path stroke-linecap="round" stroke-linejoin="round" d="m9 15 6-6"/><path stroke-linecap="round" stroke-linejoin="round" d="M9.5 10.5h.01M14.5 14.5h.01"/></svg>',
...
if (!function_exists('ui_menu_items')) {
    function ui_menu_items($pdo = null) {
        $role = $_SESSION['role'] ?? '';
        $subscription = null;

        if ($pdo instanceof PDO && !empty($_SESSION['tenant_id'])) {
            $subscription = kasirrapi_tenant_subscription($pdo, (int)$_SESSION['tenant_id']);
        }

        $items = [
            ['Dashboard', app_url('dashboard/index.php'), 'dashboard', ['Owner','Admin','Gudang','Kasir','HRD'], null],
            ['Kasir', app_url('kasir/index.php'), 'kasir', ['Owner','Admin','Kasir'], 'kasir'],
            ['Barang', app_url('barang/index.php'), 'barang', ['Owner','Admin','Gudang'], 'barang'],
            ['Piutang', app_url('piutang/index.php'), 'piutang', ['Owner','Admin','Kasir'], 'piutang'],
            ['Laporan', app_url('laporan/index.php'), 'laporan', ['Owner','Admin'], null],
            ['Absensi', app_url('absensi/index.php'), 'absensi', ['Owner','Admin','Gudang','Kasir','HRD'], 'absensi'],
            ['Gaji', app_url('gaji/index.php'), 'gaji', ['Owner','Admin','HRD'], 'gaji'],
            ['Notifikasi', app_url('notifikasi/index.php'), 'notifikasi', ['Owner','Admin','Gudang','Kasir','HRD'], null],
            ['Karyawan', app_url('karyawan/index.php'), 'karyawan', ['Owner'], null],
            ['Diskon', app_url('pengaturan/diskon.php'), 'diskon', ['Owner','Admin'], null],
            ['Pengaturan', app_url('pengaturan/toko.php'), 'pengaturan', ['Owner','Admin'], null],

            // Menu baru Profil Saya
            ['Profil Saya', app_url('profil/index.php'), 'profil', ['Owner','Admin','Gudang','Kasir','HRD'], null],

            ['Logout', app_url('auth/logout.php'), 'logout', ['Owner','Admin','Gudang','Kasir','HRD'], null],
        ];
        ...
        foreach ($items as $i) {
            $feature = $i[4] ?? null;
            if ($subscription && $feature && !kasirrapi_feature_allowed($subscription, $feature)) {
                continue;
            }
...
if (!function_exists('ui_head')) {
    function ui_head($title = null, $enableLiveChat = false) {
...
<link rel="apple-touch-icon" href="'.h(asset_url('app/icon-192.png')).'?v='.h(APP_VERSION).'">
<script src="https://cdn.tailwindcss.com"></script>
<link rel="stylesheet" href="'.h(asset_url('app/app-ui.css')).'?v='.h(APP_VERSION).'">
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>';
        $enableInstallPrompt = isset($_SESSION['tenant_id'], $_SESSION['user_id'], $_SESSION['role']);
        pwa_head_tags($enableInstallPrompt);
        if ($enableLiveChat) {
            live_chat_head_tags();
        }
        echo '
...
if (!function_exists('ui_nav')) {
    function ui_nav($pdo, $title = 'Dashboard') {
        ...
        $items = ui_menu_items($pdo);
        $activeTitle = (string)$title;
        $currentPath = '/' . trim((string)parse_url($_SERVER['SCRIPT_NAME'] ?? '', PHP_URL_PATH), '/');
    ...
    <aside id="sidebar" class="app-sidebar fixed md:sticky md:top-0 inset-y-0 left-0 z-40 w-[82%] max-w-[230px] bg-slate-950 border-r border-slate-800 transform -translate-x-full md:translate-x-0 transition-transform duration-200 no-print">
        ...
        foreach ($items as $it) {
            $itemPath = '/' . trim((string)parse_url($it['url'], PHP_URL_PATH), '/');
            $itemDir = preg_replace('#/index\.php$#', '', $itemPath);
            $isSameSection = !in_array($it['label'], ['Dashboard', 'Logout'], true)
                && $itemDir !== ''
                && ($currentPath === $itemPath || str_starts_with($currentPath, $itemDir . '/'));
