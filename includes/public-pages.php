...
<head>
    ...
    <style>
        ...
        .action-btn{min-height:44px;border-radius:14px;padding:11px 16px;font-weight:900;display:inline-flex;align-items:center;justify-content:center}
        .action-primary{background:#ff6a00;color:#fff}
        .action-secondary{background:#1e293b;color:#e2e8f0;border:1px solid #334155}
                ...
                <?php if (!empty($page['actions'])): ?>
                    <div class="action-row">
                        <?php foreach ($page['actions'] as $index => $action): ?>
                            <a href="<?= h($action['url'] ?? '#') ?>" class="action-btn <?= $index === 0 ? 'action-primary' : 'action-secondary' ?>">
                                <?= h($action['label'] ?? 'Lihat') ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
